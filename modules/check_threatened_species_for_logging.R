...
check_threatened_species_for_logging <- function(dataframe) {
    ...
    df_output <- dataframe %>%
        filter(
            categoria2 == "Explorar" &
                (nome_cientifico %in% decreto5975 | nome_aceito %in% decreto5975 |
    ...
    check_typo <- function() {
        typo <- df_output[df_output$nome_aceito != "", ]
        if (nrow(typo) > 0) {
    ...
    if (nrow(df_output) > 0) {
        if (length(unique(df_output$nome_aceito)) == 1) {
            if (check_typo()) {
                output_text <- paste0(
                    ...
                    ", no entanto, esta espécie é proibida de corte por estar Em Perigo de Extinção. Registra-se nesta análise que há erro de digitação, a saber: ",
                ...
                output_text <- paste0(
                    ...
                    ", no entanto, esta espécie é proibida de corte por estar Em Perigo de Extinção."
                )
                save_text(output_text)
            }
        } else if (length(unique(df_output$nome_aceito)) > 1) {
            if (check_typo()) {
                if (length(species_typo) == 1) {
                    output_text <- paste0(
                        "No inventário florestal constam árvores selecionadas para corte das espécies ",
                        glue::glue_collapse(df_output$nome_cientifico, sep = ", ", last = " e "),
                        ", no entanto, estas espécies são proibidas de corte por estarem Em Perigo de Extinção. Registra-se nesta análise que há erro de digitação no inventário, a saber: ",
                        accepted_name,
                        " foi grafada de forma incorreta como ",
                        "\"", species_typo, "\"."
                    )
                ...
                output_text <- paste0(
                    "No inventário florestal constam árvores selecionadas para corte das espécies ",
                    glue::glue_collapse(unique(df_output$nome_cientifico), sep = ", ", last = " e "),
                    ", no entanto, estas espécies são proibidas de corte por estarem Em Perigo de Extinção."
                )
                save_text(output_text)
            }
        }
