...
horizontal_structure <- function(df) {
        ## Get some information from the data
        umf <- paste0(unique(df$umf))
        upa <- paste0(unique(df$upa))
        flona <- paste0(unique(df$flona))
        ...
        fi_horiz <- df %>%
                ...
                row_count(count = NA) %>%
                mutate(n_UT = sum(abs(rowcount - length(unique(df$ut))))) %>%
                
                ## Set absolute density
                mutate(DA = n_UT / sum(aem$aem)) %>%
                
                ## Set absolute frequency
                mutate(FA = n_UT / length(unique(df$ut))) %>%
                select(-c(tidyr::matches('[0-9]')))
        
        
        n_sp <- df %>% group_by(nome_cientifico) %>% summarise(n = n())
